...
type DealerFilterOption = {
  id: string;
  name: string;
};

type OfferCatalogFilterOptions = {
...
type PersistedSearchState = {
  ...
  year: number;
  trim: string;
  dealerSites: DealerSiteInput[];
  includeInTransit: boolean;
  includePreSold: boolean;
...
function parseWorkspaceView(value: string | null): WorkspaceView {
  if (value === "alerts" || value === "saved-searches" || value === "sessions" || value === "inventory" || value === "copilot") {
    return value;
  }
  return "offers";
}
...
function parseNegotiationFilter(value: string | null): string {
  const normalized = (value ?? "").trim().toLowerCase();
  return normalized || "all";
}

function parseUiTheme(value: string | null): UiTheme {
  if (value === "sunset" || value === "graphite") {
    return value;
  }
  return "neon";
}
...
export function ComparisonPage() {
  ...
  useEffect(() => {
    ...
    if (persisted) {
      setUserZip(persisted.userZip ?? "18706");
      setRadiusMiles(Number.isFinite(persisted.radiusMiles) ? persisted.radiusMiles : 100);
      setBudgetOtd(Number.isFinite(persisted.budgetOtd) ? persisted.budgetOtd : 30000);
      setMake(persisted.make || "Honda");
      setModel(persisted.model || MAKE_MODEL_OPTIONS.Honda[0]);
      setYear(Number.isFinite(persisted.year) ? persisted.year : 2025);
      setTrim(persisted.trim ?? "");
      setDealerSites(Array.isArray(persisted.dealerSites) ? persisted.dealerSites : []);
      setIncludeInTransit(persisted.includeInTransit ?? true);
      setIncludePreSold(persisted.includePreSold ?? false);
      setIncludeHidden(persisted.includeHidden ?? false);
      setSelectedSavedSearchId(persisted.selectedSavedSearchId ?? null);
      setOffers(Array.isArray(persisted.offers) ? persisted.offers : []);
  ...
  const [alertTypeFilter, setAlertTypeFilter] = useState(initialUrlParams.get("alertType") ?? "all");
  const [includeAcknowledgedAlerts, setIncludeAcknowledgedAlerts] = useState(() =>
    parseBoolParam(initialUrlParams.get("includeAck"), false),
  );
  const [alertsPage, setAlertsPage] = useState(() => parsePositiveInt(initialUrlParams.get("alertsPage"), 1));
  const [alertsPageSize, setAlertsPageSize] = useState(() => parsePositiveInt(initialUrlParams.get("alertsPageSize"), 10));
  const [alertsTotal, setAlertsTotal] = useState(0);
  const [alertsTotalPages, setAlertsTotalPages] = useState(1);
  const [alertsCollapsed, setAlertsCollapsed] = useState(() => parseBoolParam(initialUrlParams.get("alertsCollapsed"), false));
  const [savedSearchesCollapsed, setSavedSearchesCollapsed] = useState(() =>
    parseBoolParam(initialUrlParams.get("savedSearchesCollapsed"), false),
  );
  const [vehicleSearchCollapsed, setVehicleSearchCollapsed] = useState(false);
  ...
  const [negotiationStatusFilter, setNegotiationStatusFilter] = useState(() =>
    parseNegotiationFilter(initialUrlParams.get("negotiationStatus")),
  );
  ...
  const [copilotUseLiveWeb, setCopilotUseLiveWeb] = useState(true);
  ...
  const [catalogPage, setCatalogPage] = useState(() => parsePositiveInt(initialUrlParams.get("catalogPage"), 1));
  ...
  useEffect(() => {
    ...
    params.set("includeAck", includeAcknowledgedAlerts ? "1" : "0");
    params.set("alertsPage", String(alertsPage));
    params.set("alertsPageSize", String(alertsPageSize));
    params.set("negotiationStatus", negotiationStatusFilter);
    params.set("alertsCollapsed", alertsCollapsed ? "1" : "0");
    ...
    if (workspaceView === "alerts") {
      void fetchAlerts();
    } else if (workspaceView === "saved-searches") {
      void fetchSavedSearches();
  ...
  async function copyVinToClipboard(vin?: string): Promise<void> {
    ...
    try {
      ...
      setCopiedVin(value);
      window.setTimeout(() => {
        setCopiedVin((current) => (current === value ? null : current));
      }, 1800);
    } catch {
  ...
  async function fetchCatalogOffers() {
    ...
    try {
      ...
      if (catalogMaxOtd.trim()) params.set("max_otd", catalogMaxOtd.trim());
      if (catalogMinDom.trim()) params.set("min_dom", catalogMinDom.trim());
      if (catalogMaxDom.trim()) params.set("max_dom", catalogMaxDom.trim());

      const response = await fetch(`${apiBase}/offers/catalog?${params.toString()}`);
      if (!response.ok) {
  ...
  async function submitCopilotMessage(rawMessage: string): Promise<void> {
    ...
    const updateAssistantMessage = (patch: Partial<AssistantChatMessage>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(await buildApiErrorMessage(response, "Copilot request failed"));
      }
        ...
        for (const block of events) {
          ...
          if (eventName === "delta") {
            ...
            const finalSuggestions = Array.isArray(parsed.suggestions)
              ...
              : [];
            const finalCitations = Array.isArray(parsed.cited_offer_ids)
              ? parsed.cited_offer_ids.filter((item): item is string => typeof item === "string")
              : [];
            const finalCheckedUrls = Array.isArray(parsed.checked_urls)
              ? parsed.checked_urls.filter((item): item is string => typeof item === "string")
              : [];
            const finalModel = typeof parsed.model === "string" ? parsed.model : undefined;

            updateAssistantMessage({
              content: finalAnswer,
              citedOfferIds: finalCitations,
  ...
  function handleCopilotSubmit(event: FormEvent<HTMLFormElement>): void {
  ...
  const isOffersView = workspaceView === "offers";
  const isAlertsView = workspaceView === "alerts";
  const isSavedSearchesView = workspaceView === "saved-searches";
  const isSessionsView = workspaceView === "sessions";
  const isInventoryView = workspaceView === "inventory";
  const isCopilotView = workspaceView === "copilot";

  useEffect(() => {
    const shouldScroll = isCopilotView || copilotDrawerOpen;
    if (!shouldScroll) {
      return;
    }

    const el = copilotDrawerThreadRef.current;
    if (!el) {
      return;
    }
  ...
  }, [isCopilotView, copilotDrawerOpen, chatMessages]);

  useEffect(() => {
    if (copilotDrawerOpen || isCopilotView) {
      setCopilotNudgeVisible(false);
      return;
    }

    const dismissed = window.sessionStorage.getItem(COPILOT_NUDGE_STORAGE_KEY) === "1";
    if (dismissed) {
      return;
    }

    const timer = window.setTimeout(() => {
      setCopilotNudgeVisible(true);
    }, 22000);

    return () => window.clearTimeout(timer);
  }, [copilotDrawerOpen, isCopilotView]);

  function openCopilotDrawer(): void {
  ...
  function startNewCopilotChat(): void {
    setChatMessages([createCopilotWelcomeMessage()]);
    setChatSuggestions([]);
    setChatInput("");
  }

  function buildWarRoomHref(sessionId: string): string {
    const current = new URLSearchParams(window.location.search);
    current.delete("view");
    current.delete("sessionId");
    const returnTo = `/?${current.toString()}`;

    const target = new URLSearchParams();
    target.set("view", "warroom");
    ...
    return (
      <nav className={`workspace-nav ${extraClassName}`.trim()} aria-label="Workspace sections">
        <button
          ...
          type="button"
          onClick={() => setWorkspaceView("offers")}
        >
          Offers
        </button>
        <button
          className={`workspace-tab ${isAlertsView ? "active" : ""}`.trim()}
          type="button"
        ...
        <button
          className={`workspace-tab ${isSavedSearchesView ? "active" : ""}`.trim()}
          type="button"
          onClick={() => setWorkspaceView("saved-searches")}
        >
          Saved Searches
          <span>{savedSearches.length}</span>
        </button>
        <button
          className={`workspace-tab ${isSessionsView ? "active" : ""}`.trim()}
          type="button"
        ...
        <button
          className={`workspace-tab ${isInventoryView ? "active" : ""}`.trim()}
          type="button"
        ...
        <button
          className={`workspace-tab ${isCopilotView ? "active" : ""}`.trim()}
          type="button"
          onClick={() => setWorkspaceView("copilot")}
        >
          AI Copilot
  ...
  return (
    <main className="comparison-page">
      ...
      <section className="panel">
        {isCopilotView && (
        <div className="panel stacked-panel workspace-panel workspace-copilot">
          <div className="panel-header">
            <h2>AI Copilot</h2>
            <span className="pill">Offers {offers.length} | Ranked {rankedOffers.length}</span>
          </div>

          <div className="copilot-thread">
            {chatMessages.map((item) => (
              <article className={`copilot-msg ${item.role}`} key={item.id}>
                <div className="copilot-msg-head">
                  <strong>{item.role === "assistant" ? "Copilot" : "You"}</strong>
                  <span>{formatSeenAt(item.createdAt)}</span>
                </div>
                <p>{item.content || (item.role === "assistant" ? "..." : "")}</p>
                {item.role === "assistant" && (
                  item.model ||
                  (item.citedOfferIds && item.citedOfferIds.length > 0) ||
                  (item.checkedUrls && item.checkedUrls.length > 0)
                ) && (
                  <div className="copilot-meta">
                    {item.model && <span className="pill">Model {item.model}</span>}
                    {item.citedOfferIds && item.citedOfferIds.map((offerId) => (
                      <span className="pill" key={`${item.id}-${offerId}`}>Offer {offerId}</span>
                    ))}
                    {item.checkedUrls && item.checkedUrls.slice(0, 3).map((url, idx) => (
                      <a className="pill" href={url} target="_blank" rel="noreferrer" key={`${item.id}-url-${idx}`}>
                        Checked Site {idx + 1}
                      </a>
                    ))}
                  </div>
            ...
            <div className="copilot-suggestions">
              {chatSuggestions.map((suggestion) => (
                <button
                  ...
                  className="btn"
                  type="button"
                  onClick={() => void submitCopilotMessage(suggestion)}
          ...
          <form className="copilot-form" onSubmit={handleCopilotSubmit}>
            <label className="wide">
              ...
              <input
                ...
                placeholder="Example: Check dealer website for this trim and verify availability"
              />
            </label>
        ...
        {isOffersView && (
          ...
          <div className="panel-head-actions">
            ...
            <button className="btn section-toggle" type="button" onClick={() => setVehicleSearchCollapsed((v) => !v)}>
              {vehicleSearchCollapsed ? "Expand" : "Collapse"}
            </button>
          </div>
        </div>

        {!vehicleSearchCollapsed && (
          <>
        ...
        <div className="actions">
          ...
          {negotiations[0] ? (
            <a className="btn link" href={buildWarRoomHref(negotiations[0].id)}>
        ...
        {isAlertsView && (
        <div className="panel stacked-panel workspace-panel workspace-alerts">
          <div className="panel-header">
            <h2>Active Alerts</h2>
        ...
        {isSavedSearchesView && (
        <div className="panel stacked-panel workspace-panel workspace-saved">
          <div className="panel-header">
        ...
        {isSessionsView && (
        <div className="panel stacked-panel workspace-panel workspace-sessions">
          <div className="panel-header">
              ...
              {filteredNegotiations.slice(0, 12).map((session) => {
                ...
                <div className="offer-card" key={session.id}>
                  ...
                  <div className="link-actions">
                    ...
                    <a className="btn link" href={buildWarRoomHref(session.id)}>
                      Open Command Center
                    </a>
                  </div>
                </div>
        ...
        {isInventoryView && (
        <div className="panel stacked-panel workspace-panel workspace-inventory">
          <div className="panel-header">
          ...
          <div className="form-grid">
            ...
            <label>
              ...
              <input
                ...
                value={catalogMinOtd}
                onChange={(e) => {
                  setCatalogMinOtd(e.target.value);
                  setCatalogPage(1);
                }}
                placeholder="25000"
              />
            ...
            <label>
              ...
              <input
                ...
                value={catalogMaxOtd}
                onChange={(e) => {
                  setCatalogMaxOtd(e.target.value);
                  setCatalogPage(1);
                }}
                placeholder="50000"
              />
            ...
            <label>
              ...
              <input
                ...
                value={catalogMinDom}
                onChange={(e) => {
                  setCatalogMinDom(e.target.value);
                  setCatalogPage(1);
                }}
                placeholder="0"
              />
            ...
            <label>
              ...
              <input
                ...
                value={catalogMaxDom}
                onChange={(e) => {
                  setCatalogMaxDom(e.target.value);
                  setCatalogPage(1);
                }}
                placeholder="120"
              />
          ...
          <div className="actions actions-reset">
            <label>
              ...
              <select
                ...
                onChange={(e) => {
                  setCatalogPageSize(Number(e.target.value) || 20);
                  setCatalogPage(1);
                }}
              >
                <option value={10}>10</option>
          ...
          {catalogTotal > 0 && (
            <div className="actions actions-spread">
              ...
              <span className="pill">Page {catalogPage} / {catalogTotalPages}</span>
              <button
                className="btn"
                type="button"
        ...
        {isOffersView && (
        <div className="panel stacked-panel workspace-panel workspace-dealers">
          <div className="panel-header">
            ...
            <div className="offer-list">
                ...
                return (
                  <div className={`rank-card${item.rank === 1 ? " best" : ""}`} key={`${item.offer.offer_id}-${item.rank}`}>
                    ...
                    <div className="stats">
                      <span title="Out-the-door total estimate including vehicle price, fees, and applicable adjustments.">
                        OTD ${item.offer.otd_price.toLocaleString()}
                      </span>
                      {item.offer.msrp !== undefined && <span title="Manufacturer suggested retail price from source API.">MSRP ${item.offer.msrp.toLocaleString()}</span>}
                      {item.offer.advertised_price !== undefined && <span title="Advertised dealer price from source API.">Adv ${item.offer.advertised_price.toLocaleString()}</span>}
                      {item.offer.selling_price !== undefined && <span title="Selling price field from source API.">Selling ${item.offer.selling_price.toLocaleString()}</span>}
                      ...
                      <span title="Vehicle feature/fit sub-score used by ranking.">Specs {item.score_breakdown.specs_score ?? 0}</span>
                      <span title="Fee sub-score used by ranking. Higher typically means a better fee profile.">
                      ...
                      {item.offer.price_drop_7d !== undefined && item.offer.price_drop_7d > 0 && (
                        <span title="Observed out-the-door price decrease over the last 7 days.">
                          Drop 7d ${item.offer.price_drop_7d.toLocaleString()}
                        </span>
                      )}
                      {item.offer.price_drop_30d !== undefined && item.offer.price_drop_30d > 0 && (
                        <span title="Observed out-the-door price decrease over the last 30 days.">
                          Drop 30d ${item.offer.price_drop_30d.toLocaleString()}
                        </span>
                      )}
                      <span title="Trend sub-score from market-age and price-movement signals.">
                        Trend {item.score_breakdown.trend_score ?? 0}
                      </span>
                    </div>
                    <div className="signal-grid">
                      <div className="signal-meter">
                    ...
                    <div className="link-actions">
                      ...
                      {activeSession && (
                        <>
                          ...
                          <a className="btn link" href={buildWarRoomHref(activeSession.id)}>
                            Open Session
                          </a>
                        </>
                      )}
      ...
      <aside className={`copilot-drawer ${copilotDrawerOpen ? "open" : ""}`.trim()} aria-live="polite">
        <div className="copilot-drawer-head">
          <strong>AI Copilot</strong>
          <div className="actions actions-tight actions-reset">
            <button className="btn" type="button" onClick={startNewCopilotChat}>
              New Chat
            </button>
            <button className="btn" type="button" onClick={() => setCopilotDrawerOpen(false)}>
              Close
            </button>
          </div>
        </div>

        <div className="copilot-thread" ref={copilotDrawerThreadRef}>
          {chatMessages.map((item) => (
            <article className={`copilot-msg ${item.role}`} key={`drawer-${item.id}`}>
              <div className="copilot-msg-head">
                <strong>{item.role === "assistant" ? "Copilot" : "You"}</strong>
                <span>{formatSeenAt(item.createdAt)}</span>
              </div>
              ...
              {item.role === "assistant" && (
                ...
                <div className="copilot-meta">
                  {item.model && <span className="pill">Model {item.model}</span>}
                  {item.citedOfferIds && item.citedOfferIds.map((offerId) => (
                    <span className="pill" key={`drawer-${item.id}-${offerId}`}>Offer {offerId}</span>
                  ))}
                  {item.checkedUrls && item.checkedUrls.slice(0, 2).map((url, idx) => (
                    <a className="pill" href={url} target="_blank" rel="noreferrer" key={`drawer-${item.id}-url-${idx}`}>
                      Checked Site {idx + 1}
                    </a>
                  ))}
                </div>
        ...
        <form className="copilot-form" onSubmit={handleCopilotSubmit}>
          <label className="wide">
            ...
            <input
              value={chatInput}
              onChange={(e) => setChatInput(e.target.value)}
              placeholder="Need me to compare and draft your next message?"
            />
          </label>
          <div className="actions actions-tight">
            <label className="toggle">
      ...
      {copilotNudgeVisible && !copilotDrawerOpen && (
        <div className="copilot-nudge" role="status">
          <p>Need help comparing offers or checking dealer sites?</p>
          <div className="actions actions-tight">
            <button className="btn primary" type="button" onClick={openCopilotDrawer}>Open Copilot</button>
            <button className="btn" type="button" onClick={dismissCopilotNudge}>Dismiss</button>
          </div>
        </div>
      )}
